...
// phi^n/sqrt(5)>10^999 solve for n.
...
/* Lets do this one another time, get from file until , trim ", add to array, MATH.
#include <iostream>
#include <vector>
#include <sstream>
...
int main(){
    ...
    // Build map of pairs for each number up to 10000
    for(int i=1; i<10000;i++){
        //std::cout<<i<<std::endl;
        if(sumOfDivisors(sumOfDivisors(i))==i && (sumOfDivisors(i))!=i){
            std::cout<<(sumOfDivisors(i))<<":"<<i<<std::endl;
            sumTotal+=i;
        }
    }
    std::cout<<sumTotal;
...
for(int a = div.size()-2;a>=0;a--){
        for(int b = div[a].size()-1; b>=0;b--) {
            //std::cout<<a<<":"<<b<<std::endl;
            div[a][b] += max(div[a+1][b],div[a+1][b+1]);
        }
